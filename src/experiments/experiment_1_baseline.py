...
def run_baseline():
    audio = pd.read_csv(AUDIO_FILE).rename(columns={"Participant_ID": "id"}).set_index("id")
    text = pd.read_csv(TEXT_FILE).rename(columns={"Participant_ID": "id"}).set_index("id")
    labels = pd.read_csv(LABELS_FILE).rename(columns={"Participant_ID": "id"}).set_index("id")
